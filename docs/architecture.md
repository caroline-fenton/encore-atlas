...
- Added `supabase/functions/_shared/ticketmaster.ts` (`pickAttraction`, `normalizeShows`, types) — pure helpers reusing `normalizeArtistName` from `src/utils/artistNameFilters.ts`, with the punctuation-only raw-match fallback from the MusicBrainz helpers. `normalizeShows` dedupes to one row per venue-night and sorts by local date. Covered by `ticketmaster.test.ts` (runs under the existing `npm test` glob).
- Frontend: `src/services/upcomingShows.ts` invokes the function with a 12h localStorage cache (successes only — errors resolve to empty and are not cached); `src/hooks/useUpcomingShows.ts` follows the `useArtistBio` hook pattern; `src/components/liveShows/UpcomingShowsSection.tsx` renders up to 4 ticket-stub rows plus an "All tour dates" link, returning `null` while loading or when empty.
