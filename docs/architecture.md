...
### 2026-07-04 — Ticketmaster upcoming shows (edge function + sidebar section)
- Added `supabase/functions/upcoming-shows/index.ts`: resolves an `artist_name` to a Ticketmaster attraction (Discovery API keyword search + strict name matching), then fetches its next events (`sort=date,asc`, anchored at now, max 20). Requires the `TICKETMASTER_API_KEY` edge-function secret; when unset it returns an empty list so pages render normally.
- Added `supabase/functions/_shared/ticketmaster.ts` (`pickAttraction`, `normalizeShows`, types) — pure helpers reusing `normalizeArtistName` from `src/utils/artistNameFilters.ts`, with the punctuation-only raw-match fallback from the MusicBrainz helpers. `normalizeShows` dedupes to one row per venue-night, drops canceled/postponed events (rescheduled are kept — their start date is updated), and sorts by local date. Covered by `ticketmaster.test.ts` (runs under the existing `npm test` glob).
- Frontend: `src/services/upcomingShows.ts` invokes the function with a 12h localStorage cache (successes only — errors resolve to empty and are not cached); `src/hooks/useUpcomingShows.ts` follows the `useArtistBio` hook pattern and is called once per layout (both responsive copies of the section are CSS-hidden, not conditionally mounted, so a self-fetching section would double the calls); `src/components/liveShows/UpcomingShowsSection.tsx` is presentational, rendering up to 4 ticket-stub rows plus an "All tour dates" link and returning `null` while loading or when empty.
- Wired into both `StandardArtistPage` and `EpicArtistPage` sidebars between `SameVibeSection` and `MerchSidebar` (and above Merch in the mobile `lg:hidden` block).
