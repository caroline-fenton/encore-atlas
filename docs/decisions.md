...
**Decision:** Two-layer fix, both deferred to the prevention PR. (1) Retrieval: append the artist's primary genre to the secondary search queries (`"Destroyer indie rock interview"`) in both `build-artist-page` and `admin-content-refresh` — the `claudeTag` result is already in scope when those searches run, so this is free. (2) Judgment: extend the existing `claudeTag` prompt (which already receives all candidate titles plus the Wikipedia extract) with per-video channel names and a new output field listing indexes of videos that don't appear to be this artist; filter those before persisting. A static `"band"` keyword was considered and rejected: wrong for solo artists and unhelpful for band-vs-band collisions (Kittie vs Kitty, Pig Destroyer vs Destroyer), which genre discriminates.

**Rationale:** The judgment layer reuses an API call that already happens on every build — no added latency or cost — and the audit is direct evidence it would have caught essentially every flagged video. Retrieval and judgment are complementary: better queries raise the floor, the Claude check catches the leftovers. Conservative stance where grounding is weak: null `wikipedia_extract` plus low-confidence should flag for review, not silently drop.
