-- Publishing manually edited metadata curates the artist, while video refreshes
-- remain available for curated artists.
...
begin
  ...
  if 'metadata' = any(refresh_row.scopes) then
    ...
    set tags = coalesce(array(select jsonb_array_elements_text(proposed_artist->'tags')), '{}'::text[]),
  ...
  if 'same_vibe' = any(refresh_row.scopes) then
    update public.artists
    set related_artists = coalesce(array(select jsonb_array_elements_text(proposed_artist->'related_artists')), '{}'::text[]),
        artist_context = jsonb_set(
  ...
  if manual_metadata_edit and refresh_row.scopes && array['metadata', 'same_vibe']::text[] then
    update public.artists set is_curated = true where id = refresh_row.artist_id;
  end if;
  ...
  if 'videos' = any(refresh_row.scopes) then
    ...
    if exists (
      select 1 from jsonb_array_elements_text(manual_video_replacements) replacement
      where not exists (
        select 1 from public.artist_videos existing
    ...
    if exists (
      ...
      where existing.artist_id = refresh_row.artist_id
        ...
        and (
          exists (
            select 1 from jsonb_array_elements(proposed_videos) proposed
    ...
    where existing.artist_id = refresh_row.artist_id
      and coalesce(existing.video_type, 'concert') = 'concert'
      and (not existing.is_manually_added or (manual_video_replacements ? existing.youtube_video_id))
      and not exists (
        select 1 from jsonb_array_elements(proposed_videos) proposed
        where proposed->>'youtube_video_id' = existing.youtube_video_id
      );
    ...
    insert into public.artist_videos (
      artist_id, youtube_video_id, title, description, thumbnail_url,
      published_at, view_count, duration, search_query, is_manually_added,
      display_order, video_type, channel_title
    )
    select
      refresh_row.artist_id, proposed.youtube_video_id, proposed.title,
      proposed.description, proposed.thumbnail_url, proposed.published_at,
      proposed.view_count, proposed.duration, proposed.search_query,
      proposed.is_manually_added, proposed.display_order, 'concert',
      proposed.channel_title
    from jsonb_to_recordset(proposed_videos) as proposed(
      youtube_video_id text, title text, description text, thumbnail_url text,
      published_at timestamptz, view_count bigint, duration text,
      search_query text, is_manually_added boolean, display_order integer,
      channel_title text
    )
    ...
    set title = excluded.title,
        ...
        is_manually_added = public.artist_videos.is_manually_added or excluded.is_manually_added,
  ...
  set status = 'published', published_snapshot = published, published_at = now(), error_message = null
  where id = p_refresh_id;
